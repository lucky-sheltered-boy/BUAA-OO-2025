...
public class FunctionDefiner {
    ...
    public static void addNormalFunc(String s1) {
        String s = Preprocessing.process(s1);
        String name = s.charAt(0) + "";
        funcMap.put(name, new ArrayList<String>());
        ArrayList<String> para = getFuncPara(s);
        paraMap.put(name, para);
        funcMap.get(name).add(getFuncBody(s));
    }
    ...
    public static String parseFuncn(String strn, ArrayList<BigInteger> coe,
        ...
        while (start < strn.length() && !(stack == 1 && strn.charAt(start) == ',')) {
