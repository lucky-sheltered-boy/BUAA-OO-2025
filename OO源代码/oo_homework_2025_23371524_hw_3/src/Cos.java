...
public class Cos implements Factor {
    ...
    public Poly dx() {
        if (exp.compareTo(BigInteger.ZERO) == 0) {
            ...
            sinList.add(sin);
            Mono mono = new Mono(new BigInteger("-1"),BigInteger.ZERO,sinList,new ArrayList<Cos>());
            ...
            Cos cos = new Cos(exp.subtract(BigInteger.ONE), poly.clone());
            ArrayList<Cos> cosList = new ArrayList<>();
            cosList.add(cos);
            Mono mono = new Mono(exp, BigInteger.ZERO,new ArrayList<Sin>(),cosList);
            ArrayList<Mono> monoList = new ArrayList<Mono>();
            monoList.add(mono);
            Poly poly = new Poly(monoList);
            Cos chain = new Cos(BigInteger.ONE, this.poly.clone());
